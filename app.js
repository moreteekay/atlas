...
L.tileLayer(\
  ...
  \{\
    attribution:"\'a9 OpenStreetMap contributors \'a9 CARTO",\
    subdomains:"abcd",\
    maxZoom:20\
  \}\
).addTo(map);\
...
function buildStory(record)\{\
  ...
  const timeline=text(p.Timeline)||[p["First Visit"],p["Last Visit"]].map(text).filter(Boolean).join("\'96");\
\
...
\
  if(search.value.trim()||activeTab!=="places"||activeFilter!=="All")\{\
    navigation.innerHTML=visible.length?visible.slice(0,150).map((r,i)=>`<button class="nav-item" data-item="$\{i\}"><strong>$\{esc(r.place.Place)\}</strong><span>$\{esc([r.place.Type,r.place.Region,r.place.Country].map(text).filter(Boolean).join(" \'b7 "))\}</span></button>`).join(""):"<p>No places found.</p>";\
